...
## Conceptualising Interaction — Week 6 Part 1
...
### AT3 Design Stage Context

This lecture sits within the **Ideate** phase of the AT3 design process:

Empathize→Define→Ideate⏟← you are here→Prototype→Test\text{Empathize} \rightarrow \text{Define} \rightarrow \underbrace{\text{Ideate}}_{\text{← you are here}} \rightarrow \text{Prototype} \rightarrow \text{Test}Empathize→Define→← you are hereIdeate​​→Prototype→Test

Conceptual models are a key **ideation output** before moving into prototyping.
...
- The loop of: feel→do→[system]→feel→do→…\text{feel} \rightarrow \text{do} \rightarrow [\text{system}] \rightarrow \text{feel} \rightarrow \text{do} \rightarrow \ldots feel→do→[system]→feel→do→…
...
- Design implication: we **cannot assume** all users think about a system the same way
...
- Lead to frustrating behaviours: mashing keys, repeated clicking, frozen cursor responses

#### Mental Model Exercises

> [!example] Exercise 1 — Thermostat You get home late, your place is steaming hot, you want to cool it down as fast as possible.
> 
...
> [!example] Exercise 2 — Pedestrian Crossing Button You're late and need to cross the road. The light just turned green for traffic.
> 
...
> [!important] Key Insight Mental models are not always accurate, but they **drive user behaviour regardless**.

---

### Conceptual Models

#### Why Mental Models Aren't Enough for Designers

While mental models belong to individual users and describe existing knowledge, they are **not directly useful for interaction designers** — you can't design a mental model, you can only _influence_ one.
...
_Based on: Mental and Conceptual Models in design, Alana Brajdic, UX Collective_

The designer communicates their conceptual model **via design principles** → shapes the interface → which in turn shapes and trains the user's mental model.
...
> [!tip] Exam/Assignment Hint — These four steps are directly assessable
...
**Preece et al. propose four main interaction types:** Instructing, Conversing, Manipulating, Exploring.
...
> [!warning] Hybrid interaction types are often employed but can be **harder to learn**.
